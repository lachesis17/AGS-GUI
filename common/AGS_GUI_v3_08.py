...
def main():
    ...
    class Application(ct.CTkFrame):
        ...
        def __init__(self):
            ...
            window.wm_iconbitmap(bitmap='common/images/geo.ico', default='common/images/geo.ico')
            window.lift()
            window.geometry('450x440+150+150')
            window.resizable(False,False)
            window.title("AGS GUI v3.08")
                        ...
                        if not test_type == "":
                            ...
                            if not lab_count_none == 0:
                                ...
                                if lab_count_off == 0 and not lab_count_on == 0:
                                    count = [f"Total:{count}, Onshore:{lab_count_on}"]
                                elif lab_count_on == 0 and not lab_count_off == 0:
                                    count = [f"Total:{count}, Offshore:{lab_count_off}"]
                                else:
                                    count = [f"Total:{count}, Offshore:{lab_count_off}, Onshore:{lab_count_on}"]
                        else:
                            ...
                            #print(f"{table} offshore: {lab_count_off} onshore: {lab_count_on} no lab: {lab_count_none}")
                            if not lab_count_none == 0:
                                count = [f"Total:{count}, Offshore:{lab_count_off}, Onshore:{lab_count_on}, None:{lab_count_none}"]
                            else:
                                if lab_count_off == 0 and not lab_count_on == 0:
                                    count = [f"Total:{count}, Onshore:{lab_count_on}"]
                                elif lab_count_on == 0 and not lab_count_off == 0:
